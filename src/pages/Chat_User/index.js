...
import getChatUser from '../../components/services/api-chat/GetChatUserId';
...
const Chat = () => {
  ...
  useEffect(() => {
    const fetchChatUsers = async () => {
      try {
        const response = await getChatUser(idUsers);
        ...
        {chat.map((option, index) => (
          <div style={{display: 'flex'}} key={index}>
            <Link onClick={() => handleCardClick(option.user_id)}>
              <CardContainer>
                <CardTitle>Porteiro</CardTitle>
                <CardText>Mensagem:{option.description}</CardText>
